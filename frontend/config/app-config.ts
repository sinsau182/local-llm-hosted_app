...
export type AppConfig = {
  features: {
    ...
    /**
     * Toggle for the Audio tab + /audio route (Whisper STT + Kokoro TTS).
     *  - true  → the Audio link is shown in the header nav.
     *  - false → the Audio link is hidden.
     */
    audioPage: boolean;
  };
  /**
...
export const appConfig: AppConfig = {
  features: {
    ...
    // Source of truth: NEXT_PUBLIC_FEATURE_AUDIO_PAGE in the root master .env.
    // true = Audio tab (Whisper STT + Kokoro TTS) shown in the header nav.
    audioPage: envFlag(process.env.NEXT_PUBLIC_FEATURE_AUDIO_PAGE, true),
  },
  ...
  comfyuiUrl:
    ...
    "https://breachlabz-nucbox-evo-x2.tail040b45.ts.net:8443",
  ...
  models: [
    // LibreChat card. Toggle with NEXT_PUBLIC_FEATURE_LIBRECHAT in the root
    // master .env (false hides it from the launcher). Defaults to shown.
    ...(envFlag(process.env.NEXT_PUBLIC_FEATURE_LIBRECHAT, true)
      ? ([
          {
            label: "LibreChat",
